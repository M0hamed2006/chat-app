...
async function callAPIStream(provider, model, messages, onChunk, signal) {
  ...
  while (attempt < 3) {
    ...
    } catch (err) { if (err.name === 'AbortError') throw err; attempt++; if (attempt >= 3) throw err; await sleep(delay); delay *= 2; }
  }
}
...
async function sendChat(text, isRetry = false) {
  ...
  try {
    ...
    removeThinking(); const errorMsg = `فشل الرد: ${lastError?.message || 'غير معروف'}`; addErrorWithRetry(errorMsg, () => sendChat(finalText, true)); addToChat('assistant', `❌ ${errorMsg}`); el.apiState.textContent = '⚡ ❌ خطأ';
