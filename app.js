// ============================================================
//  المصري الذكي – النسخة الأسطورية v10.0
//  إصلاح شامل، لصق صور، نطق، أداء ذكي
// ============================================================
...
const START_MESSAGE = `✨ أهلاً! أنا **المصري الذكي** v10.0 – دعم لصق الصور، نطق، وذكاء فائق. اسأل أي شيء! 🚀🦅`;

// ========== 4. UTILITIES (تعريف كل الدوال) ==========
...
const sanitizeHTML = (html) => {
  if (window.DOMPurify) return window.DOMPurify.sanitize(html, { ALLOWED_TAGS: ['strong', 'em', 'code', 'pre', 'h1', 'h2', 'h3', 'ul', 'li', 'a', 'p', 'br', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'img'], ALLOWED_ATTR: ['href', 'target', 'rel', 'src', 'alt', 'class'] });
  return html;
};
...
async function extractTextFromImage(dataUrl) {
  ...
  try {
    ...
    if (statusEl) statusEl.textContent = '❌ فشل تحليل الصورة';
    return '';
  }
}
...
setInterval(() => {
  const now = Date.now();
  for (let [k, v] of responseCache.entries()) if (now > v.expiry) responseCache.delete(k);
}, 3600000);
...
const el = {
  ...
  bgNextBtn: $('bgNextBtn'), bgAutoBtn: $('bgAutoBtn'), customModelInput: $('customModelInput'), imageOcrStatus: $('imageOcrStatus')
};

// ========== 7. BACKGROUND SYSTEM (كل 15 ثانية) ==========
let _activeLayer = 1;
...
// ========== 8. TABS ==========
const PANEL_META = { chat: ['🦅 دردشة ذكية', 'لصق صور + نطق + ذكاء'], search: ['🔎 بحث عميق', 'ويكيبيديا + جوجل'], image: ['🎨 الصور الذكية', 'رسم + بحث'], settings: ['⚙️ الإعدادات', 'مفاتيح + تحذيرات'] };
...
function renderModelNotes() { el.modelNotes.value = [`المزود: ${currentProvider().label}`, '⚠️ تخزين المفاتيح غير آمن', `عدد الخلفيات: ${BACKGROUNDS.length}`, 'تم تفعيل OCR ولصق الصور'].join('\n'); }
...
function refreshKeyUI() { const has = !!currentKey(); el.keyState.textContent = has ? '🔑 ✅' : '🔑 ❌'; el.apiState.textContent = has ? '⚡ جاهز' : '⚡ يحتاج مفتاح'; el.groqKeyInput.value = state.groqKey; el.openaiKeyInput.value = state.openaiKey; setStatus(el.keyStatus, has ? 'good' : 'warn', has ? '✅ مفتاح محفوظ (غير آمن)' : '⚠️ أدخل مفتاح API أولاً من الإعدادات.'); }
...
// ========== 11. RENDER MESSAGES مع Markdown ونطق ==========
function addMsgToUI(role, content, meta = {}) {
  ...
  if (role === 'assistant' && meta.isStreaming) {
    ...
    const safeHtml = sanitizeHTML(md.render(content));
  ...
  // زر النطق
  if (role === 'assistant' && !meta.isStreaming && content && content.trim().length > 0 && window.speechSynthesis) {
...
function addErrorWithRetry(errorMsg, retryCallback) {
  ...
  const bubble = document.createElement('div'); bubble.className = 'msg-bubble';
  bubble.innerHTML = sanitizeHTML(md.render(`❌ ${errorMsg}\n\n[حاول مرة أخرى]($)`));
  const retryLink = bubble.querySelector('a');
  if (retryLink) { retryLink.addEventListener('click', (e) => { e.preventDefault(); retryCallback(); }); retryLink.style.cursor = 'pointer'; retryLink.style.textDecoration = 'underline'; }
  wrap.appendChild(avatar); wrap.appendChild(bubble);
  el.chatMessages.appendChild(wrap);
  scrollBottom();
}
...
// ========== 12. CONVERSATIONS ==========
function renderConversationList() { state.chats.sort((a, b) => b.updatedAt - a.updatedAt); el.conversationList.innerHTML = state.chats.map(c => `<button class="conversation-item ${c.id === state.activeChatId ? 'active' : ''}" data-id="${c.id}"><div class="ci-title">${escHTML(c.title)}</div><div class="ci-meta">${c.messages.length} رسالة</div></button>`).join(''); $$('.conversation-item').forEach(btn => btn.addEventListener('click', () => switchChat(btn.dataset.id))); }
...
async function callAPIStream(provider, model, messages, onChunk, signal) {
  const prov = PROVIDERS[provider]; const key = provider === 'openai' ? state.openaiKey : state.groqKey;
  if (!key) throw new Error('API key missing');
  const cacheKey = getCacheKey(provider, model, messages); const cached = getCached(cacheKey);
  ...
  while (attempt < 3) {
    try {
      const res = await fetch(prov.endpoint, {
        ...
        body: JSON.stringify({ model, messages, temperature: 0.85, max_tokens: 2500, top_p: 0.95, stream: true }),
        signal
      });
      if (res.status === 401) throw new Error('مفتاح API غير صالح (401)');
      ...
      setCached(cacheKey, fullReply); onChunk(fullReply, true); return fullReply;
    } catch (err) {
      ...
      if (file) {
        ...
        reader.onload = async (ev) => {
          ...
          img.onload = () => {
            ...
            state.attachedImage = { name: 'pasted_image.png', dataUrl: canvas.toDataURL('image/jpeg', 0.8) };
            updateAttachmentBox();
            // تشغيل OCR تلقائي
            const statusEl = document.getElementById('imageOcrStatus');
            if (statusEl) statusEl.textContent = '⏳ تحليل الصورة الملصوقة...';
            extractTextFromImage(state.attachedImage.dataUrl).then(ocr => {
              if (ocr && statusEl) statusEl.textContent = '✅ تم التحليل (OCR)';
              state.ocrText = ocr; // تخزين مؤقت للـ OCR
            });
          };
          img.src = dataUrl;
        };
...
async function sendChat(text, isRetry = false) {
  ...
  if (!currentKey()) {
    addMsgToUI('user', text); addToChat('user', text);
    addMsgToUI('assistant', '⚠️ أضف مفتاح API أولاً من الإعدادات (Groq مجاني).'); addToChat('assistant', '⚠️ أضف مفتاح API');
    return;
  }
  ...
  try {
    ...
    let ocrText = state.ocrText || '';
    if (img && !ocrText) {
      ocrText = await extractTextFromImage(img.dataUrl);
      state.ocrText = ocrText;
    }
    addMsgToUI('user', text, { imageDataUrl: img?.dataUrl, imageName: img?.name }); addToChat('user', text, { imageDataUrl: img?.dataUrl, imageName: img?.name });
    addThinkingIndicator(); await sleep(300);
    const chat = getActiveChat(); const candidates = getModelCandidates(text, !!img); let lastError = null, success = false;
    for (const model of candidates) {
      try {
        const msgs = buildMessages(chat, text, img, ocrText);
        removeThinking();
        const bubble = addMsgToUI('assistant', '', { isStreaming: true });
        const streamId = bubble.id;
        let accumulated = '';
        const onChunk = (chunk, isEnd) => {
          if (chunk && chunk !== accumulated) {
            accumulated = chunk;
            const bubbleEl = document.getElementById(streamId);
            if (bubbleEl) {
              const safeHtml = sanitizeHTML(md.render(accumulated));
              bubbleEl.innerHTML = safeHtml;
              if (window.Prism) Prism.highlightAllUnder(bubbleEl);
              scrollBottom();
            }
          }
          if (isEnd && accumulated) { addToChat('assistant', accumulated); success = true; }
        };
        ...
        if (success) {
          ...
          state.attachedImage = null; state.ocrText = ''; updateAttachmentBox();
          return;
        }
      } catch (err) { if (err.name !== 'AbortError') lastError = err; removeThinking(); continue; }
    }
    removeThinking();
    const errorMsg = `فشل الرد: ${lastError?.message || 'غير معروف'}. تأكد من صحة مفتاح API.`;
    addErrorWithRetry(errorMsg, () => sendChat(text, true));
...
el.googleSearchBtn.addEventListener('click', () => { const q = el.searchInput.value.trim(); if (q) window.open(`https://www.google.com/search?q=${encodeURIComponent(q)}`, '_blank', 'noopener noreferrer'); });
el.genBtn.addEventListener('click', generateImage); el.imagePrompt.addEventListener('keydown', e => { if (e.key === 'Enter') generateImage(); });
el.webImageBtn.addEventListener('click', () => { const q = el.imagePrompt.value.trim(); if (q) window.open(`https://www.google.com/search?q=${encodeURIComponent(q)}&tbm=isch`, '_blank', 'noopener noreferrer'); });
...
(function init() {
  ...
  el.chatInput.focus();
  console.log(`🦅 v10.0 – لصق صور، نطق، ذكاء – ${BACKGROUNDS.length} خلفية`);
})();
